...
class AsyncFlagManager:
    ...
    def with_context(self, context: Context) -> "AsyncFlagManager":
        clone = copy.copy(self)
        clone._context = context
        return clone

    def with_defaults(self, defaults: DefaultsCollection) -> "AsyncFlagManager":
        clone = copy.copy(self)
        clone._defaults = defaults
        return clone

    async def report_usage(self, key: str, context: Optional[Context] = None) -> None:
        await self._api_client.report_usage(key, context)
